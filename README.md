...
1. 拖入单独音频或使用文件选择器并执行“完整识别”，`paraformer-zh + fsmn-vad` 会生成
   逐字 PCM 时间戳。完整转写按 `fsmn-vad` 检测到的人声段逐段换行，每行开头显示该段
...
4. 文字时间戳只提供初始建议；右侧橙色时间范围才是最终实际删除范围。建立标注前后都可
   拖动波形边界或修改毫秒值，调整结果自动保存。
5. 右侧统一提供“听原音（前后 3 秒）”“只听删除”“试听删除后”。前后对比均保留切点
...
两个工作区的选择、分析、试听、保存和导出状态都会同步显示在窗口左下角状态栏。
