...
- **Word 黄标剪辑**：读取带有 `发言人 HH:MM` 时间锚点的 DOCX，把黄色高亮文字
  对齐到音频，经过人工复核后导出。
- **音频处理**：只导入一段音频，完整识别为带时间戳文字；用户选择文字或框选波形建立
...
2. 使用随安装包交付的 `paraformer-zh + fsmn-vad` 先识别整段音频的真实讲话和绝对
   时间戳，再通过“唯一锚点 + VAD/段落约束的单调 DP”把完整 Word 正文映射到真实语音；
...
3. 只有时间戳单位、范围和单调性均有效，存在真实逐字时间与原始模型置信度，且候选差距、
   相邻字符保护和双模型一致性都达标时才可自动通过。单字/双字、上下文重复、粗时间戳、
   置信度缺失、局部覆盖不足或双模型边界差异超过 120 ms 的项目必须人工处理。
4. 对每个不确定片段可分别“听原句”“只听待删”“听剪后”，在约 5 ms/点的局部波形上
   ...
   零交叉处理；发生外扩会重新进入复核。所有待复核项处理完前禁止导出。
...
1. 拖入单独音频或使用文件选择器并执行“完整识别”，`paraformer-zh + fsmn-vad` 会保留
   模型实际提供的 PCM 时间戳精度。完整转写按 `fsmn-vad` 检测到的人声段逐段换行，每行开头显示该段
   的音频开始时间；旧项目会依据逐字停顿和最长 30 秒段长自动补充分段。
2. 在完整转写中拖选文字，然后通过按钮、右键、`Delete/Backspace`、`Ctrl+B` 或 macOS
   `⌘B` 建立删除标注。重叠标注会安全合并。
3. 选择文字或点击已有标注时，文字范围、波形和毫秒输入框同步。波形支持直接框选、拖动
   两侧边界、滚轮缩放以及 `Alt+拖动` 平移。
4. 文字时间戳只提供初始建议；右侧橙色时间范围才是最终实际删除范围。词/句级时间戳、
